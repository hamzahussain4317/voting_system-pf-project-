...
//To extract rollno from userID -- For example, userID:2018btecs00064 rollno:00064.
...
void createCandidateFiles(){
    ...
    for( i = 1;i <= numberOfCandidates; i++){
		...
		fp=fopen(filename,"w");
        fprintf(fp,"               \nNAME OF CANDIDATE : %s",candidateArray[i-1].cname);
		fclose(fp);
    }
...
int main(){
	...
	for(i=0;i<3;i++){
		...
		printf("\t\t\t\t\t\tWELCOME TO THE DIGITAL VOTING SYSTEM %c\n" ,b);
	for(i=0;i<3;i++){
			for(j=0;j<150;j++){
				printf("%c" ,a);
	}puts("");
